...
def getCrates(stacks: str):
    ...
    # the same as: stacks = [elem[1] for elem in level.split()] for level in stacks]
...
def moveStack(crates: str, line: str):
    ...
    if (nCrates == 1):
        ...
        values = [crates[srcStack].pop() for i in range(nCrates)]

        while (nCrates > 0):
            crates[dstStack].append(values.pop())
            nCrates -= 1
